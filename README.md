...
This Terraform module deploys the
[aws-securityhubv2-bot](https://github.com/cruxstack/aws-securityhubv2-bot), a
Go-based Lambda function that automatically processes AWS Security Hub findings
in the OCSF (Open Cybersecurity Schema Framework) format. The bot can suppress,
resolve, or archive findings based on configurable rules and optionally send
notifications to Slack with rich context.

The bot is built and packaged automatically during `terraform apply` using
[BuildKit](https://github.com/moby/buildkit) from the source repository. You can
specify a specific version or use `latest` to deploy from the main branch.
...
This module requires **Terraform >= 1.7** or **OpenTofu >= 1.7**.

A **BuildKit** endpoint must be reachable from the machine executing Terraform.
The module uses the
[cruxstack/buildkit](https://registry.terraform.io/providers/cruxstack/buildkit/latest)
provider to build and package the Lambda function from the
[aws-securityhubv2-bot](https://github.com/cruxstack/aws-securityhubv2-bot)
repository during `terraform apply`. The provider auto-discovers the BuildKit
endpoint embedded in Docker Desktop, OrbStack, or Colima, honors the
`BUILDKIT_HOST` environment variable, and can supervise an embedded rootless
`buildkitd` on Linux. See the
[provider documentation](https://registry.terraform.io/providers/cruxstack/buildkit/latest/docs)
...
Rules are defined using the `rules` variable. For large rule sets (>3KB), enable
S3 storage:
...
- Uploads rules defined in the `rules` variable to S3 as individual JSON files
- Configures appropriate IAM permissions for the Lambda function

You can also use an existing S3 bucket by setting `create = false` and providing
the bucket name.
...
Based on
[OCSF 1.6.0 specification](https://schema.ocsf.io/1.6.0/classes/detection_finding):
...
Common usage: `status_id: 5` (Archived) for accepted behavior, `status_id: 4`
(Resolved) for remediated issues, `status_id: 3` (Suppressed) for false
positives.
...
| bot_force_rebuild_id      | ID to force rebuilding the Lambda function source code. Change this value to trigger a rebuild.                                      | string       | ""                                                       | no       |
...
Additional CloudPosse context variables (`namespace`, `environment`, `stage`,
`name`, `enabled`, `delimiter`, `attributes`, `tags`, etc.) are inherited from
the `cloudposse/label/null` module. See [context.tf](./context.tf) for details.
